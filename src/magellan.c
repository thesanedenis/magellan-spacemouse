...
#ifdef MAGELLAN_TARGET_RS232_ADAPTER
...
volatile bool magellan_needs_init = false;
...
int main(void)
{
    ...
    while (1)
    {
        ...
        if (magellan_needs_init) {
            magellan_init();
            magellan_needs_init = false;
        }

        uint32_t now = to_ms_since_boot(get_absolute_time());

        // Long press logic for '*' button (index 8)
